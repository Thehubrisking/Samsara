...
const getSpatialHeader = (maskPresent: boolean) => 
    `[MASTER MODIFICATION PROTOCOL ACTIVE]:
     ...
     ${maskPresent ? 'MODIFICATION TARGET: Input 1 (Selection Mask). Only modify area within these pixels.' : ''}
     CONSULTATIVE CUES: All subsequent inputs are visual references (Character, Outfit, Pose, or Scene).
     
     CRITICAL DIRECTIVES:
     1. MANDATORY OUTPUT: You MUST return a single transformed image part. Do NOT return only text.
     2. GROUND TRUTH: Analyze "Input 0" for skeletal anatomy, pose, and spatial context. 
     3. SYNTHESIS: Apply stylistic/structural data from reference inputs onto Input 0 while maintaining its fundamental identity.
     4. NO MODIFICATION: Do NOT modify or return the reference images. Transform ONLY Input 0.`;
